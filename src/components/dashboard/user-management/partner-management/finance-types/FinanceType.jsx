...
const FinanceTypeModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://naijatrips-app-dcarbon-server.cafyit.easypanel.host/api/admin/financial-types', {
        method: 'POST',
        headers: {
...
const EditFinanceTypeModal = ({ isOpen, onClose, financeType, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://naijatrips-app-dcarbon-server.cafyit.easypanel.host/api/admin/financial-types/${financeType.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`
...
export default function FinanceTypes({ onBack, onViewChange }) {
  ...
  const fetchFinanceTypes = async () => {
    try {
      setLoading(true);
      const authToken = localStorage.getItem('authToken');
      const response = await fetch('https://naijatrips-app-dcarbon-server.cafyit.easypanel.host/api/user/financial-types', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
  ...
  const handleApprove = async (id) => {
    try {
      ...
      const response = await fetch(`https://naijatrips-app-dcarbon-server.cafyit.easypanel.host/api/admin/financial-types/${id}/review`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await fetch(`https://naijatrips-app-dcarbon-server.cafyit.easypanel.host/api/admin/financial-types/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
